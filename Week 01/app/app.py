...
if 'ISBN' not in st.session_state:
    # Default ISBN to kickstart the app
...
with info:
    ...
    st.caption(
        f"{df_book['Year-Of-Publication'].iloc[0]} | {df_book['Publisher'].iloc[0]}")
...
data_path = 'recommendations/recommendations-seeded-freq.csv'
if os.path.exists(data_path):
    df = pd.read_csv(data_path, sep=';', encoding='latin-1', dtype=object)
    isbn = st.session_state['ISBN']
    df_recommendations = df[df['book_a'] == isbn].sort_values(
...
st.subheader(
    'Recommendations based on Frequently Reviewed Together (associations)')
data_path = 'recommendations/recommendations-seeded-associations.csv'
if os.path.exists(data_path):
    df = pd.read_csv('recommendations/recommendations-seeded-associations.csv',
                    sep=';', encoding='latin-1', dtype=object)
    isbn = st.session_state['ISBN']
    df_recommendations = df[df['source'] == isbn].sort_values(
